...
if(DISTRO_NIX)
    file(GLOB CHUNK_FILES "${NIX_SHIMS_PATH}/chunks/*")
else()
...
if(CMAKE_BUILD_TYPE STREQUAL "Release")
    ...
    set(HEADER_CONTENT "${HEADER_CONTENT}#else\n")
    if(DISTRO_NIX)
        set(HEADER_CONTENT "${HEADER_CONTENT}INCTXT(MILLENNIUM_FRONTEND_BIN_, \"${NIX_FRONTEND_PATH}/frontend.bin\");\n")
    else()
    ...
    set(HEADER_CONTENT "${HEADER_CONTENT}#else\n")
    if(DISTRO_NIX)
        set(HEADER_CONTENT "${HEADER_CONTENT}INCTXT(MILLENNIUM_API_BIN_, \"${NIX_SHIMS_PATH}/millennium.js\");\n")
    else()
    ...
    if (DISTRO_NIX)
        set(RC_CONTENT "${RC_CONTENT}MILLENNIUM_FRONTEND_BIN RCDATA \"${NIX_FRONTEND_PATH}/frontend.bin\"\n")
        set(RC_CONTENT "${RC_CONTENT}MILLENNIUM_API_BIN RCDATA \"${NIX_SHIMS_PATH}/millennium.js\"\n\n")
    else()
...
foreach(chunk_file ${CHUNK_FILES})
    ...
    if (DISTRO_NIX)
        file(RELATIVE_PATH rel_path "${NIX_SHIMS_PATH}" "${chunk_file}")
    else()
    ...
    if(CMAKE_BUILD_TYPE STREQUAL "Release")
        ...
        if(DISTRO_NIX)
            set(CHUNK_DEFS "${CHUNK_DEFS}INCTXT(${var_name}, \"${chunk_file}\");\n")
        else()
